...
int main(){
    ...
    while (scanf("%d", &width) != 1 || width < 10 || width > 80){
        scanf("%c", &temp);
        printf("Input type is not valid, please enter the width again (10-80): ");
    }

    printf("Please enter the height (10-20):   ");
    while (scanf("%d", &height) != 1 || height < 10 || height > 20){
        scanf("%c", &temp);
        printf("Input type is not valid, please enter the height again (10-20): ");
    }

    printf("Please enter the top left RGB:     ");
    while (scanf("%d,%d,%d", &top_left_r, &top_left_g, &top_left_b) != 3 || top_left_r < 0 || top_left_r > 255 || top_left_g < 0 || top_left_g > 255 || top_left_b < 0 || top_left_b > 255){
        scanf("%c", &temp);
        printf("Please enter the top left RGB again: ");
    }
    
    printf("Please enter the top right RGB:    ");
    while (scanf("%d,%d,%d", &top_right_r, &top_right_g, &top_right_b) != 3 || top_right_r < 0 || top_right_r > 255 || top_right_g < 0 || top_right_g > 255 || top_right_b < 0 || top_right_b > 255){
        scanf("%c", &temp);
        printf("Please enter the top right RGB again: ");
    }
    
    printf("Please enter the bottom left RGB:  ");
    while (scanf("%d,%d,%d", &bottom_left_r, &bottom_left_g, &bottom_left_b) != 3 || bottom_left_r < 0 || bottom_left_r > 255 || bottom_left_g < 0 || bottom_left_g > 255 || bottom_left_b < 0 || bottom_left_b > 255){
        scanf("%c", &temp);
        printf("Please enter the bottom left RGB again: ");
    }
    
    printf("Please enter the bottom right RGB: ");
    while (scanf("%d,%d,%d", &bottom_right_r, &bottom_right_g, &bottom_right_b) != 3 || bottom_right_r < 0 || bottom_right_r > 255 || bottom_right_g < 0 || bottom_right_g > 255 || bottom_right_b < 0 || bottom_right_b > 255){
        scanf("%c", &temp);
